...
def main(model_load_location):
    ...
    test_result, overall_f1, overall_precision, overall_recall, overall_accuracy = val_testing(model, test_loader, labels_to_ids, device, tokenizer)
    ...
    return test_result, overall_f1, overall_precision, overall_recall, overall_accuracy
...
if __name__ == '__main__':
    ...
    for loop_index in range(n_rounds):
        for model_name in models:
            ...
            result_save_location = '../bert-large/20_epochs_baseline/eval_testing/saved_eval_test_result_1b/' + model_name + '/' + str(loop_index) + '/'
            report_result_save_location = '../bert-large/20_epochs_baseline/eval_testing/saved_eval_report_1b/' + model_name + '/' + str(loop_index) + '/'
            ...
            test_result, best_f1, best_precision, best_recall, best_accuracy = main(model_load_location)
    ...
    os.makedirs('../bert-large/20_epochs_baseline/eval_testing/eval_validation_statistics/', exist_ok=True)
    all_best_acc.to_csv('../bert-large/20_epochs_baseline/eval_testing/eval_validation_statistics/all_best_dev_acc.tsv', sep='\t')
    all_best_f1_score.to_csv('../bert-large/20_epochs_baseline/eval_testing/eval_validation_statistics/all_best_f1_score.tsv', sep='\t')
    all_best_precision.to_csv('../bert-large/20_epochs_baseline/eval_testing/eval_validation_statistics/all_best_precision.tsv', sep='\t')
    all_best_recall.to_csv('../bert-large/20_epochs_baseline/eval_testing/eval_validation_statistics/all_best_recall.tsv', sep='\t')
    ...
    with open('../bert-large/20_epochs_baseline/eval_testing/eval_validation_statistics/time.txt', 'w') as file:
