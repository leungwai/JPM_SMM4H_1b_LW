...
def find_matching_token(batch_prediction_df, tokenizer):
    ...
    for index, row in batch_prediction_df.iterrows():
        ...
        original_span_string = tokenizer.convert_tokens_to_string(original_span_token)[1:]
        predicted_span_string = tokenizer.convert_tokens_to_string(predicted_span_token)[1:]
        ...
        if predicted_span_token == []:
            ...
            first_word_in_predicted_span_token = predicted_span_token[0][1:]
            last_word_in_predicted_span_token = predicted_span_token[-1][1:]
            print("First word in token")
            print(first_word_in_predicted_span_token)

            original_sentence = row['orig_sentence']
            span_begin = ""
            if first_word_in_predicted_span_token[0:1] != 'Ġ':
                span_begin = re.search(r"\w*%s\w*"%first_word_in_predicted_span_token, original_sentence)
            else:
                first_word_in_predicted_span_token = first_word_in_predicted_span_token[1:]
            ...
            if first_word_in_predicted_span_token == last_word_in_predicted_span_token:
                ...
                if last_word_in_predicted_span_token[0:1] != 'Ġ':
                    span_end = re.search(r"\w*%s\w*"%last_word_in_predicted_span_token, rest_of_sentence)
                else:
                    last_word_in_predicted_span_token = last_word_in_predicted_span_token[1:]
...
if __name__ == '__main__':
    ...
    for loop_index in range(n_rounds):
        for model_name in models:
            ...
            model_save_location = '../roberta-base/saved_models_1b/' + model_name + '/' + str(loop_index) + '/' 
            model_load_location = None

            epoch_save_location = '../roberta-base/saved_epoch_1b/' + model_name + '/' + str(loop_index) + '/' 
            epoch_save_name = epoch_save_location + '/epoch_info.tsv'

            result_save_location = '../roberta-base/saved_data_1b/' + model_name + '/' + str(loop_index) + '/'

            report_result_save_location = '../roberta-base/saved_report_1b/' + model_name + '/' + str(loop_index)
    ...
    os.makedirs('../roberta-base/validating_statistics/', exist_ok=True)
    all_best_dev_acc.to_csv('../roberta-base/validating_statistics/all_best_dev_acc.tsv', sep='\t')
    all_best_f1_score.to_csv('../roberta-base/validating_statistics/all_best_f1_score.tsv', sep='\t')
    all_best_precision.to_csv('../roberta-base/validating_statistics/all_best_precision.tsv', sep='\t')
    all_best_recall.to_csv('../roberta-base/validating_statistics/all_best_recall.tsv', sep='\t')
    ...
    with open('../roberta-base/validating_statistics/time.txt', 'w') as file:
